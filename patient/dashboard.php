...
$patient = $stmt->fetch(PDO::FETCH_ASSOC); // Ensure it returns an associative array
...
$stmt_appointments = $conn->prepare("
    SELECT a.id, d.name AS doctor_name, a.appointment_date, a.status 
    FROM appointments a
...
$appointments = $stmt_appointments->fetchAll(PDO::FETCH_ASSOC); // Ensure it returns an associative array
?>
...
<body>
    ...
    <main>
        <section>
            <h2>Welcome, <?php echo htmlspecialchars($patient['name'] ?? 'Guest'); ?>!</h2>
            <p>Email: <?php echo htmlspecialchars($patient['email'] ?? 'N/A'); ?></p>
            <p>Phone: <?php echo htmlspecialchars($patient['phone_number'] ?? 'N/A'); ?></p>
        </section>
        <section>
